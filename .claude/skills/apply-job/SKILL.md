...
- Wizard steps: My Information, My Experience, Application Questions, Voluntary Disclosures, Self Identify, Review. Click "Save and Continue" between steps; validation errors appear at the top of the step.
- Fields carry `data-automation-id` attributes; use them in `find`/JS. Country/phone-type are custom dropdowns (button opens a list).
- On the Review step stop; the button there is Submit.
