...
3. **Prepare an application.** Expand a job, choose its provider, model, and GPT thinking level, then click **Prepare application**. Your last-used choices are remembered. **Resume** defaults to the version that matches the job title (MLE for ML / AI roles, SDE otherwise); change it for that job if the title misleads. If you keep cover letter templates in `setup/cover_letter/`, a one-page cover letter tailored to the posting is written first and uploaded to any cover letter field; open it from the application to check it before you submit. Screening has its own provider setting under **AI settings**.
